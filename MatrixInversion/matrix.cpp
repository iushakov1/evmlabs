...
Matrix Matrix::operator+(const Matrix& other) const {
    ...
    for(int i = 0; i < this->row; ++i){
        for(int j = 0; j < this->column; ++j){
            float a_ij = this->getElement(i, j) + other.getElement(i, j);
            resData[i*this->column + j] = a_ij;
        }
    }
    ...
    for(int i = 0; i < this->row; ++i){
        for(int k = 0; k < this->column; ++k){
            for(int j = 0; j < other.column; ++j){
                resData[i*other.column + j] += (this->getElement(i, k) * other.getElement(k, j));
            }
        }
    }
    Matrix res(resData, resRow, resColumn);
...
void Matrix::add(Matrix& other) {
    ...
    for(int i = 0; i < this->row; ++i){
        for(int j = 0; j < this->column; ++j){
            float a_ij = this->getElement(i, j) + other.getElement(i, j);
            this->data[i*this->column + j] = a_ij;
        }
    }
}
...
void Matrix::mul(Matrix& other, float* buf) {
    ...
    for(int i = 0; i < this->row; ++i){
        for(int k = 0; k < this->column; ++k){
            for(int j = 0; j < other.column; ++j){
                buf[i*other.column + j] += (this->getElement(i, k) * other.getElement(k, j));
            }
        }
    }
    memcpy(this->data, buf, this->row * other.column * sizeof(float));
    memset(buf, 0, this->row * other.column * sizeof(float));
}